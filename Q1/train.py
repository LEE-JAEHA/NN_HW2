...
def train():
    ...
    parser.add_argument("--model_save", default="./checkpoint/checkpoint_f_B512_ep200_lr0.01", help="model save folder")
    parser.add_argument("--model_load", default="./checkpoint/checkpoint_b_B512_ep200/model_149.pth", help="model load folder")
    ...
    if args.pretrained ==True:
        ...
        data = torch.load(args.model_load)
    ...
    q_b = ["linear"]
    q_c = ["linear","layer4"] # train conv5
    q_d = ["linear", "layer4","layer3"]  # train conv5 conv4
    q_f = ["linear","linear2"]  # train all
    for name,para in model.named_parameters():
        if name.split(".")[0] in q_f:
            print(name)
            para.requires_grad = True
            continue
        para.requires_grad = False  # freeze
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch >1 :
            save_name = "./checkpoint/{0}/model_{1}.pth".format(model_save,epoch)
            save_parameters(model, save_name, optimizer)
    plot_graph(epochs,(train_loss,test_loss),(train_accuracy,test_accuracy),str(best_acc))
